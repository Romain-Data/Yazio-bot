...
from app.models.analysis import RepasAnalysis, Aliment
...
def _handle_equivalence_creation(analysis: RepasAnalysis) -> dict:
    eq_key = analysis.equivalence_key or "Inconnu"
    eq_val = analysis.equivalence_value or "0g"
    mammouth_service.save_custom_weight(eq_key, eq_val)
...
def _handle_recipe_creation(analysis: RepasAnalysis) -> dict:
    recipe_name = analysis.nom_recette or "Recette personnalisée"
    portions = analysis.portions or 1
    macros = yazio_service.create_recipe(recipe_name, portions, analysis.aliments)
...
def _handle_food_logging(analysis: RepasAnalysis) -> dict:
    results = []
    for aliment in analysis.aliments:
        if getattr(aliment, "is_recipe", False):
            recipe_match = yazio_service.search_recipe(aliment.nom)
